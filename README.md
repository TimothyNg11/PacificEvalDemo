...
- ✅ **Fix `raptor_qcond`** — done; see [docs/results/qcond_v2/](docs/results/qcond_v2/comparison.md). All three candidate mechanisms were implemented (`tau_level_scale`, `leaf_bias`, `expand_terminal_leaves` in `QCondConfig`) and calibrated with a free retrieval-only sweep ([scripts/sweep_qcond.py](scripts/sweep_qcond.py)); the diagnostic showed beam width (`k_branch`), not termination, was the binding constraint. v2 reaches parity with collapsed search on held-out QASPER. Remaining idea: test the termination mechanisms on deeper trees (larger corpora), where tree levels — and level-dependent termination — should matter more than on these depth-2/3 trees.
